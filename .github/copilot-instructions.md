...
> ## 🔓 WHEN MCP IS OPTIONAL — Manual / Context-Only Mode
>
> The MCP rules below apply **only when working with D365FO X++ objects** (`.xml` / `.xpp` files, AOT objects, label files, project files).
>
> **You do NOT need MCP tools for:**
> - Non-D365FO files: `*.cs`, `*.json`, `*.yml`, `*.md`, `*.config`, `*.csproj`, `*.sln`, plain text
> - Reading / reviewing code without making changes (user just wants to understand something)
> - The user explicitly says: *"just read the file"*, *"show me the content"*, *"I'll handle the context"*, *"no MCP"*
> - Working in the MCP server repository itself (TypeScript source)
>
> ⚠️ **`.rnrproj` vs `.csproj` — critical distinction:**
> - **`.rnrproj`** is the D365FO Visual Studio project file — it IS managed by MCP (`create_d365fo_file` with `addToProject=true`). **NEVER edit `.rnrproj` directly** with built-in tools — the MCP tool handles adding/updating entries correctly.
> - **`.csproj`** is a C# project file (test projects, interop) — it is NOT managed by MCP and can be edited with built-in tools.
>
> **In these cases you may freely use built-in tools** (`read_file`, `code_search`, `edit_file`, etc.) — the MCP prohibition only covers D365FO object files.
>
> **If the user wants to disable MCP guidance for a session**, they can say:
> - *"Use manual mode"* / *"Skip MCP"* / *"Use built-in tools"*
> - After this, treat MCP tools as optional (use them only if they genuinely help), and use built-in tools freely.
> - The PowerShell prohibition still applies — Developer PowerShell hangs regardless of mode.
