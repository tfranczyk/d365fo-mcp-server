...
export const TOOL_ANNOTATIONS: Record<string, ToolAnnotations> = {
  ...
  // confirm_implementation_plan writes nothing to disk — it only flips an
  // in-memory session flag that unlocks the code-creating tool actions. Marked
  // read-only so clients don't prompt for it (zero extra clicks in the approval flow).
  confirm_implementation_plan:      read('Confirm implementation plan'),
